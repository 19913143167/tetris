...
public class PreviewPanel extends JPanel implements PreviewListener, ConfigListener {
	...
	public void paintShape(Graphics g, Shape shape) {
		...
		g.setColor(Config.getPreviewColor(shape.getType()));
		...
		for (int i = 0; i < length; i++) {
			...
			g.fill3DRect((1 + points[i][0]) * Config.PREVIEW_CELL_WIDTH,
					(1 + points[i][1]) * Config.PREVIEW_CELL_HEIGHT, 
					Config.PREVIEW_CELL_WIDTH, Config.PREVIEW_CELL_HEIGHT, true);
		}
	...
	private void paintBackGround(Graphics g) {
		...
		// 设置背景色
		int cellWidth = Config.PREVIEW_CELL_WIDTH;
		int cellHeight = Config.PREVIEW_CELL_HEIGHT;
		int width = Config.PREVIEW_WIDTH * cellWidth;
		int height = Config.PREVIEW_HEIGHT * cellHeight;
		g.fillRect(0, 0, width, height);
		// 绘制网格
		if(Config.SHOW_GRID_LINE_PREVIEW) {
			...
			// 绘制水平线
			for (int i = 1; i <= Config.PREVIEW_HEIGHT; i++) {
				g.drawLine(0, i * cellHeight - 1, width - 1, i * cellHeight - 1);
			}
			// 绘制垂直线
			for (int i = 1; i <= Config.PREVIEW_WIDTH; i++) {
