...
public class GamePanel extends JPanel 
	...
	private void paintGround(Graphics g, Ground ground) {
		...
		for (int i = 0; i < length; i++) {
			...
			g.setColor(Config.getGroundColor(points[i][2]));
	...
	private void paintGround(Graphics g, Shape shape) {
		...
		_paintShape(g, shape, Config.getGroundColor(shape.getType()));
	}
	...
	private void paintShape(Graphics g, Shape shape) {
		if(shape == null) return;
		_paintShape(g, shape, Config.getShapeColor(shape.getType()));
	}
	...
	private void _paintGroundLine(Graphics g, Ground ground, int line, Color color) {
		...
		for (int i = 0; i < length; i++) {
			...
			g.setColor(color == null ? Config.getGroundColor(points[i][2]) : color);
			_paintPoint(g, points[i][0], points[i][1]);
		}
	...
	private void _paintPoint(Graphics g, int x, int y) {
		g.fill3DRect(x * Config.CELL_WIDTH,
				y * Config.CELL_HEIGHT, 
				Config.CELL_WIDTH, Config.CELL_HEIGHT, true);
	}
	...
	protected void paintBackGround(Graphics g) {
		...
		// 设置背景色
		int cellWidth = Config.CELL_WIDTH;
		int cellHeight = Config.CELL_HEIGHT;
		int width = Config.GROUND_WIDTH * cellWidth;
		int height = Config.GROUND_HEIGHT * cellHeight;
		g.fillRect(0, 0, width, height);
		// 绘制网格
		if(Config.SHOW_GRID_LINE) {
			g.setColor(Config.COLOR_GRID_LINE);
			// 如果有边框, 不绘制第一条水平线和垂直线
			if(getBorder() == null) {
			...
			// 绘制水平线
			for (int i = 1; i <= Config.GROUND_HEIGHT; i++) {
				g.drawLine(0, i * cellHeight - 1, width - 1, i * cellHeight - 1);
			}
			// 绘制垂直线
			for (int i = 1; i <= Config.GROUND_WIDTH; i++) {
	...
	protected void paintCovering(Graphics g) {
		...
		// 设置颜色
		Color oldColor = g.getColor();
		g.setColor(!playing ? Config.COLOR_COVERING_OVER : Config.COLOR_COVERING_PAUSEA);
		g.fillRect(0, 0, width, height);
	...
	public void groundFilledShape(final Ground ground, final Shape shape) {
		this.ground = ground;
		this.shape = null;
		if(Config.SUPPORT_COLORFUL_SHAPE != Config.SUPPORT_COLORFUL_GROUND)
		...
		for(int i = 0; i < 2; i++) {
			...
			synchronized (this) {
				...
				for(int j = 0; j < length; j++) {
					_paintGroundLine(g, ground, fullLine[j], Config.COLOR_WINK);
				}
				try {
					Thread.sleep(Config.WINK_PAUSE_TIME);
				} catch (InterruptedException e) {
					e.printStackTrace();
				}
				for(int j = 0; j < length; j++) {
	...
	public void groundWillClear(final Ground ground) {
		...
		do {
			...
			synchronized(this) {
				// 显示障碍物的闪烁效果
				for(int i = 0; i < 2; i++) {
					this._paintGroundLine(g, copy, copy.getHeight() - 1, Config.COLOR_WINK);
					try {
						Thread.sleep(Config.WINK_PAUSE_TIME);
					} catch (InterruptedException e) {
						e.printStackTrace();
