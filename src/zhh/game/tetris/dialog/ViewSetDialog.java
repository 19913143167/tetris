...
public class ViewSetDialog extends JDialog {
	...
	public ViewSetDialog(Frame owner, boolean modal) {
		...
		choose[0] = Config.SHOW_GRID_LINE;
		choose[1] = Config.SHOW_GRID_LINE_PREVIEW;
		choose[2] = Config.SUPPORT_COLORFUL_SHAPE;
		choose[3] = Config.SUPPORT_COLORFUL_GROUND;
		choose[4] = Config.SUPPORT_COLORFUL_PREVIEW;
		...
		for(int i = 0; i < controlCount; i++) {
			...
			labels[i].setBounds(10, 10 + i * 36, 150, 15);
			getContentPane().add(labels[i]);
			
			// 设置选择框
			rdoTrue[i] = new JRadioButton();
			rdoTrue[i].setText(i < 2 ? "显示" : "彩色");
			rdoTrue[i].setBounds(40, 25 + i * 36, 65, 19);
			...
			rdoFalse[i].setBounds(116, 25 + i * 36, 65, 19);
		...
		btnOk.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				// 检查配置是否已经被改变
				if(Config.SHOW_GRID_LINE != choose[0] ||
		        		Config.SHOW_GRID_LINE_PREVIEW   != choose[1] ||
		        		Config.SUPPORT_COLORFUL_SHAPE   != choose[2] || 
		        		Config.SUPPORT_COLORFUL_GROUND  != choose[3] ||
		        		Config.SUPPORT_COLORFUL_PREVIEW != choose[4]) {
					// 更新配置
					Config.SHOW_GRID_LINE           = choose[0];
					Config.SHOW_GRID_LINE_PREVIEW   = choose[1];
					Config.SUPPORT_COLORFUL_SHAPE   = choose[2];
					Config.SUPPORT_COLORFUL_GROUND  = choose[3];
					Config.SUPPORT_COLORFUL_PREVIEW = choose[4];
