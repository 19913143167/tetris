...
public class HotkeySetDialog extends JDialog {
	...
	public HotkeySetDialog(Frame owner, boolean modal) {
		...
		keyCodes[0] = Config.KEY_START;
		keyCodes[1] = Config.KEY_PAUSE;
		keyCodes[2] = Config.KEY_LEFT;
		keyCodes[3] = Config.KEY_RIGHT;
		keyCodes[4] = Config.KEY_ROTATE;
		keyCodes[5] = Config.KEY_DOWN;
		keyCodes[6] = Config.KEY_SWIFT;
		...
		final String[] labelText = {"开始/结束:", "暂停/继续:", "左移:", "右移:", "变形:", 
				"加速下落:", "一落到底:"};
		// 文本框边框
		final Border txtBorder = new LineBorder(Config.COLOR_BORDER, 1, false);
		// 提示标签
		...
		for(int i = 0; i < controlCount; i++) {
			...
			labels[i].setBounds(10, 10 + i * 27, 70, 15);
			getContentPane().add(labels[i]);
			
			// 设置文本框
			textFields[i] = new JTextField();
			textFields[i].setText(KeyEvent.getKeyText(keyCodes[i]));
			textFields[i].setBounds(84, 7 + i * 27, 120, 21);
		...
		separator.setBounds(10, 196, 194, 2);
		...
		btnRestoration.setBounds(10, 204, 108, 23);
		btnRestoration.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				// 恢复为应用程序的默认控制键
				keyCodes[0] = Config.KEY_START_DEFAULT;
				keyCodes[1] = Config.KEY_PAUSE_DEFAULT;
				keyCodes[2] = Config.KEY_LEFT_DEFAULT;
				keyCodes[3] = Config.KEY_RIGHT_DEFAULT;
				keyCodes[4] = Config.KEY_ROTATE_DEFAULT;
				keyCodes[5] = Config.KEY_DOWN_DEFAULT;
				keyCodes[6] = Config.KEY_SWIFT_DEFAULT;
				// 更新文本框
				for(int i = 0; i < controlCount; i++) {
					textFields[i].setText(KeyEvent.getKeyText(keyCodes[i]));
		...
		btnOk.setText("确定(O)");
		btnOk.setMnemonic('O');
		btnOk.setBounds(10, 233, 108, 23);
		btnOk.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				// 检查控制键是否已经被改变
				if(Config.KEY_START != keyCodes[0]
		                || Config.KEY_PAUSE != keyCodes[1]
            			|| Config.KEY_LEFT != keyCodes[2]
            			|| Config.KEY_RIGHT != keyCodes[3]
            			|| Config.KEY_ROTATE != keyCodes[4]
            			|| Config.KEY_DOWN != keyCodes[5]
            			|| Config.KEY_SWIFT != keyCodes[6]) {
					// 更新控制键
					Config.KEY_START = keyCodes[0];
					Config.KEY_PAUSE = keyCodes[1];
					Config.KEY_LEFT = keyCodes[2];
					Config.KEY_RIGHT = keyCodes[3];
					Config.KEY_ROTATE = keyCodes[4];
		...
		btnCancel.setBounds(124, 233, 80, 23);
