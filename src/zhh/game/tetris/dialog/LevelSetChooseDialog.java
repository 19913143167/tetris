...
public class LevelSetChooseDialog extends JDialog {
	...
	public LevelSetChooseDialog(Frame owner, boolean modal, boolean isGamePlaying) {
		...
		chooseLevelSet = Config.CURRENT_LEVELSET;
		chooseInitLevel = Config.INIT_LEVEL;
		
		final JLabel lblLevelSet = new JLabel();
		lblLevelSet.setText("关卡选择:");
		lblLevelSet.setBounds(10, 10, 60, 15);
		...
		for(int i = 0; i < controlCount; i++) {
			...
			radioButtons[i].setBounds(10, i * 23, 100, 23);
		...
		// 线条形边框
		final Border lineBorder = new LineBorder(Config.COLOR_BORDER, 1, false);
		// 带滚动条的面板(在级别集很多的情况下有用)
		final JScrollPane scrollPane = new JScrollPane(panel);
		scrollPane.setBounds(30, 27, 174, 72);
		scrollPane.setBorder(lineBorder);
		getContentPane().add(scrollPane);

		final JLabel label = new JLabel();
		label.setText("级别选择:");
		label.setBounds(10, 109, 60, 15);
		getContentPane().add(label);

		// 级别选择时的文本提示
		final JTextField textField = new JTextField();
		textField.setText("" + (Config.INIT_LEVEL + 1));
		...
		separator.setBounds(10, isGamePlaying ? 196 : 225, 194, 2);
		...
		if(isGamePlaying) {
			...
			btnApplyLater.setBounds(10, 204, 108, 23);
			btnApplyLater.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					Config.CURRENT_LEVELSET = chooseLevelSet;
					Config.INIT_LEVEL = chooseInitLevel;
					dispose();
				}
			});
