...
import zhh.game.tetris.listenerinstance.ScoringAdapter;
 ...
 * @author zhaohuihua
 */
public class GameController extends KeyAdapter implements ConfigListener {
	 ...
	 * 而用户可能仍未停止操作, 导致该操作直接作用于下一形状, 但这一操作对于下一形状可能并不合适<br>
	 * 以形状创建的时间加上一个配置的偏移时间来屏蔽形状刚创建时的部分操作事件以减少这种情况<br>
	 */
	...
	public GameController() {
		...
		// 计分器增加监听器
		scorer.addScoringListener(new ScoringAdapter() {
			...
			public void levelChanged(Level level) {
				...
				// 1. 将预览形状清空<br>
				int previewListenerLen = previewListeners == null ? 0 : previewListeners.length;
				nextShape = null;
				previewListenerLen = previewListeners == null ? 0 : previewListeners.length;
				for(int i = 0; i < previewListenerLen; i++)
					previewListeners[i].shapePreviewCleared();
				
				// 2. 将障碍物清空
				//    清空前发出游戏显示事件, 以便通知显示组件显示一些效果
				int gameViewListenerLen = gameViewListeners == null ? 0 : gameViewListeners.length;
				for(int i = 0; i < gameViewListenerLen; i++)
				...
				nextShape = ShapeFactory.getRandomShape(scorer.getCurrentLevel().getComplexity());
				for(int i = 0; i < previewListenerLen; i++)
			...
			public void scoringInit(int scoring, int speed, Level level) {
				...
				if(level.getFraiseLine() > 0) {
					int length = gameViewListeners == null ? 0 : gameViewListeners.length;
					ground.randomFill(scorer.getCurrentLevel().getFraiseLine(), 
							scorer.getCurrentLevel().getFraiseFillRate());
					// 填充后发出游戏显示事件, 以便通知显示组件显示一些效果
			...
			public void winning(int scoring, int speed, Level level) {
				playing = false;
				shape = null;
	    		int length = gameListeners == null ? 0 : gameListeners.length;
				for(int i = 0; i < length; i++)
    ...
    public void keyPressed(KeyEvent e) {
    	...
    	if(keyCode == Config.KEY_START) {
    	...
    	} else if(keyCode == Config.KEY_PAUSE) {
    		if(!playing) return;
        	...
        	// 针对形状的按键事件, 如果游戏暂停或形状仍未被创建或按键的时间早于创建时间, 则抛弃之
	    	if(!playing || pause || shape == null || e.getWhen() <= shapeCreateTime)
	    		return;
			int length = gameViewListeners == null ? 0 : gameViewListeners.length;
	    	if(keyCode == Config.KEY_LEFT) {
	    		// 形状向左移动的按键处理
	    	...
	    	} else if(keyCode == Config.KEY_ROTATE) {
	    		// 形状变形的按键处理
				Shape shape = (Shape)this.shape.clone();
	    		shape.rotate();
	    		if(!ground.collisional(shape)) {
	    	...
	    	} else if(keyCode == Config.KEY_SWIFT) {
	    		// 形状一落到底的按键处理
	    		// 关掉目前的形状自动下落驱动器
	    		// 另外开启一个无间歇的形状自动下落驱动器
				SoundController.play(SoundController.SWIFT);
     ...
     * 4. 创建新形状<br>
     */
    private synchronized void shapeDrop() {
		Shape shape = (Shape)this.shape.clone();
		shape.moveDown();
		int length = gameViewListeners == null ? 0 : gameViewListeners.length;
		if(!ground.collisional(shape)) {
			...
			// 形状变成障碍物
			ground.fill(this.shape);
			SoundController.play(SoundController.DOWN);
			for(int i = 0; i < length; i++)
				gameViewListeners[i].groundFilledShape(this.ground, this.shape);
    ...
    private void shapeCreate() {
    	...
    	if(ground.collisional(nextShape)) {
			playing = false;
			shape = null;
	    	SoundController.play(SoundController.OVER);
    		int length = gameListeners == null ? 0 : gameListeners.length;
			for(int i = 0; i < length; i++)
				gameListeners[i].gameOver();
    	} else {
			...
			shapeDropDriver.start();
			nextShape = null;
			length = previewListeners == null ? 0 : previewListeners.length;
			for(int i = 0; i < length; i++)
				previewListeners[i].shapePreviewCleared();
    ...
    public void gameCreate() {
    	...
    	SoundController.play(SoundController.START);
    	...
    	// 初始化游戏环境
		if(ground == null)
			ground = new Ground(Config.GROUND_WIDTH, Config.GROUND_HEIGHT);
		else {
			ground.clear();
			length = gameViewListeners == null ? 0 : gameViewListeners.length;
			for(int i = 0; i < length; i++)
				gameViewListeners[i].groundCleared();
			length = previewListeners == null ? 0 : previewListeners.length;
			for(int i = 0; i < length; i++)
	...
	public void gamePause() {
		...
		pause = true;
    	SoundController.play(SoundController.PAUSE);
		int length = gameListeners == null ? 0 : gameListeners.length;
		for(int i = 0; i < length; i++)
	...
	public void gameContinue() {
		...
		pause = false;
    	SoundController.play(SoundController.CONTINUE);
		int length = gameListeners == null ? 0 : gameListeners.length;
		for(int i = 0; i < length; i++)
    ...
    public void addGameListener(GameListener listener) {
    	if(gameListeners == null) gameListeners = new GameListener[]{};
    ...
    public void addGameViewListener(GameViewListener listener) {
    	if(gameViewListeners == null) gameViewListeners = new GameViewListener[]{};
    ...
    public void addPreviewListener(PreviewListener listener) {
    	if(previewListeners == null) previewListeners = new PreviewListener[]{};
