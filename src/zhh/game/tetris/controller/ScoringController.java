...
public class ScoringController {
	...
	public ScoringController() {
		this.scoring = 0;
		this.level = Config.INIT_LEVEL;
		this.initLevel = Config.INIT_LEVEL;
		this.levelSet = LevelSetFactory.getLevelSet(Config.CURRENT_LEVELSET);
    ...
    public void score(int line) {
		...
		if(level == newLevel)
			SoundController.play(SoundController.SCORING);
    	int length = scoringListeners == null ? 0 : scoringListeners.length;
		for(int i = 0; i < length; i++)
			scoringListeners[i].scoringChanged(scoring);
		if(level < newLevel) {
			if(newLevel < levelSet.getLevelCount()) {
				// 级别变化了
				level = newLevel;
				SoundController.play(SoundController.LEVEL);
				...
				SoundController.play(SoundController.WINNING);
				for(int i = 0; i < length; i++)
			...
			// 当前级别的得分
			int currentLevelScoring = scoring - (level - initLevel) * levelSet.getInterval();
			// 新的速度
    ...
    public String toString() {
    	...
    	result.append("Scoring: ").append(scoring)
